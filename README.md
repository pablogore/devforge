...
| **Static analysis** | Runs golangci-lint and govulncheck as pipeline steps; repositories can add custom plugins via `.syntegrity.yml`. |
...
**Configuration priority:** For `pr`, `release`, and `doctor`, profile and mode are resolved in order: **CLI flags** override **`.syntegrity.yml`**, then **auto-detection** (profile) or **default** (mode: full).
...
Repositories can customize DevForge using a **`.syntegrity.yml`** file in the repository root. **If the file does not exist, DevForge falls back to default behavior** (profile auto-detection, mode `full`, no plugins). The file is optional.
...
Repositories can control which steps run using the **`pipeline`** key in `.syntegrity.yml`. If no pipeline section is provided, the default pipeline runs unchanged.
...
CLI flags (`--profile`, `--mode`) take precedence over values in `.syntegrity.yml`.
...
You can set the mode in `.syntegrity.yml`:
...
If `.syntegrity.yml` does not define `mode`, the default is `full`. Existing behavior is unchanged when mode is omitted.

---

## 5. Profiles

Profiles define which pipelines and thresholds apply to a repository. Profile can be set via `--profile`, `.syntegrity.yml`, or auto-detection.
...
| Pipeline | Purpose |
|----------|---------|
| **PR validation** | Runs before merge: go mod tidy, conventional commit, architectural guard, policy-pack, static analysis (golangci-lint), security (govulncheck), tests, coverage; then any plugins from `.syntegrity.yml`. Fails on first step error. |
...
These suggestions can be added manually under `.syntegrity/policies/` (e.g. create `security.yaml`, `domain.yaml`, or `architecture.yaml` with the indicated rules). If no issues are detected, doctor prints: **No policy suggestions detected.**
...
This command analyzes the repository and writes policy files under **`.syntegrity/policies/`** (e.g. `architecture.yaml`, `security.yaml`). If no issues are detected, it prints **No policies needed.** and does not create files. Generated policies are enforced by the DevForge PR pipeline (policy-pack step).

**Example workflow:**

```bash
forge doctor --generate-policies
git add .syntegrity/policies
...
| **Plugins** | Custom steps from `.syntegrity.yml` (e.g. `proto-lint`, `license-check`); each runs via `bash -c "<run>"`. |
...
Plugins are custom validation steps defined in `.syntegrity.yml` under `plugins`. Each plugin has a **name** (for logging) and a **run** command (executed via `bash -c`). Plugins run **after** the standard pipeline (static analysis, tests, coverage). If a plugin command exits with a non-zero code, the pipeline fails.

**Writing a Hello World plugin**

1. Add a plugin entry to `.syntegrity.yml` in your repo root:
...
DevForge allows repositories to extend the validation pipeline using **plugins** defined in `.syntegrity.yml`. Plugins are **simple commands executed after the core pipeline completes**.
...
3. Plugins defined in `.syntegrity.yml` execute in order.

**Example output:**

```
INFO running plugin plugin=hello-world
Hello from syntegrity plugin
...
Plugins execute **sequentially** in the order defined in `.syntegrity.yml`. If any plugin exits with a non-zero code, the pipeline fails.
...
func main() {
	fmt.Println("hello from syntegrity plugin")
}
```
...
Plugins can be configured using `.syntegrity.yml`. Use the **`plugins`** key as a map from plugin name to options (e.g. `enabled`, and any custom keys like `severity`). That configuration is passed to the plugin via the environment variable **`DEVFORGE_PLUGIN_CONFIG`** (JSON).
...
```
~/.syntegrity/plugins/<plugin>/tools
```
...
Repositories can define **governance rules** using policy packs under `.syntegrity/policies/`. Policy packs are **optional**: if the directory does not exist or is empty, no policy validation runs and the pipeline behaves exactly as it does today.
...
### Example: `.syntegrity/policies/security.yaml`
...
Policy packs run after the architectural guard step during `forge pr`. If no `.syntegrity/policies` directory exists, the step is a no-op and the pipeline is unchanged.
...
- **Detects repository type (profile)** — Infers whether the repo is `go-lib` or `go-service` (or uses profile from `.syntegrity.yml` or workflow inputs).
...
- **Executes repository plugins** — Runs any additional validation steps defined in `.syntegrity.yml` (e.g. license-check, proto-lint) after the main pipeline.
...
**What happens when the workflow runs:** The action restores or downloads the forge binary, then DevForge analyzes the repository, selects the correct pipeline profile, runs the validation pipeline (and installs tools like golangci-lint and govulncheck under `~/.syntegrity/tools` when needed), and executes any optional plugins defined in `.syntegrity.yml`. The job fails if any step in the pipeline or a plugin fails.

---

## 11. Architecture

DevForge uses a layered (hexagonal) design. The CLI loads `.syntegrity.yml` (optional) and applies flag overrides before selecting profile and invoking pipelines. Dependencies point inward: CLI → profiles → application → ports; steps implement application’s Step interface; adapters implement ports; domain has no external dependencies.
...
internal/
  ...
  config/                 Repository config loading (.syntegrity.yml)
...
- **config** — Loads `.syntegrity.yml`; used by CLI to resolve profile/mode/plugins when flags are not set. (Config file name preserved for backward compatibility.)
...
- **Minimal configuration** — Profiles encode defaults (thresholds, timeouts, step sets). Repositories can optionally add `.syntegrity.yml` to set profile, mode, and plugins; CLI flags override config; auto-detection applies when neither is set.
